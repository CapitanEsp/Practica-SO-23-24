        ...
        if(strcmp(command[1], "-set") == 0){
            ...
            if(strcmp(command[2], "-l") == 0){
                if(nargs == 4){
                    ...
                    if(newuid == -1){
                        printf("Usuario no existente %s", command[3]);
                    }else{
                        if(seteuid(newuid) != 0){
        ...
        if(nargs > 3){
            ...
            if(nargs == 2){
                if(atoi(command[1]) !=  getpid()){
                    ...
                    if(p!=PNULL){
                        i = getDataP(p, *L);
                        printf("Hola%d\t%s p=%d %s %s (%d) %s", i.pid, i.user, i.prio, i.launch, i.nsignal, i.signal, i.comandName);
                    }
                }else{  
                    jobs(command, nargs, L);
                }
